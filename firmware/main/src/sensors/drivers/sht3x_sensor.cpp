...
esp_err_t Sht3xSensor::poll() {
    ...
    esp_err_t err = sht3x_measure(&device_, &temperature_c, &humidity_percent);
    if (err != ESP_OK) {
        setError(std::string("Failed to read SHT3X measurement: ") + esp_err_to_name(err));
    ...
    if (std::isnan(temperature_c) || std::isnan(humidity_percent)) {
        setError("SHT3X driver returned invalid values.");
        if (soft_fail_policy_.onPollErr()) {
            ESP_LOGE(kTag, "hard error after %u soft fails: %s", kSensorPollFailureReinitThreshold, last_error_.c_str());
            initialized_ = false;
        } else if (soft_fail_policy_.soft_fails == 1U) {
...
std::string Sht3xSensor::lastError() const {
    return last_error_;
}
...
void Sht3xSensor::setError(const std::string& message) {
    last_error_ = message;
}
