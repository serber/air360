...
2. **Warmup phase** — the sensor is within 60 s of becoming eligible (its `eligible_since_ms`; see the startup phases in [startup-pipeline.md](startup-pipeline.md#sensor-startup-phases)). Warmup readings are intentionally excluded to avoid uploading the rapid-fire startup polls (every 5 s) as if they were real scheduled measurements. The status page and diagnostics still see them via `latest_by_sensor_`.
