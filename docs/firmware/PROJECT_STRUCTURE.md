...
- `managed_components/` — ESP-IDF component manager dependencies (bmp280, bme680, dht, ds18b20, scd30, scd4x, sht3x, sht4x, si7021, veml7700, tinygpsplusplus, esp_modem, led_strip, onewire_bus)
...
- `main/src/web/` — decomposed web route/support files: `web_form.cpp` for host-testable URL/form parsing, `web_request_body.cpp` for the size-limited request-body reader (`readRequestBodyWithRetries`), `web_runtime_routes.cpp` for read-only/runtime endpoints, `web_mutating_routes.cpp` for `/config`, `/sensors`, and `/backends`, `web_ota_routes.cpp` for the OTA upload/status/rollback endpoints, and `web_server_helpers.cpp` for HTTP request/response helpers
...
| `scd4x_sensor.cpp` | SCD40, SCD41 | `esp-idf-lib__scd4x` (managed component) |
