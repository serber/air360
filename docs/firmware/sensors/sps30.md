...
3. Acquire `SensirionI2cContextGuard` around the Sensirion library setup calls — binds the HAL pointer to this driver's `i2c_dev_t` while holding the shared Sensirion HAL mutex
...
Each poll cycle:
1. Acquire `SensirionI2cContextGuard` for the Sensirion library call — required because the vendor HAL uses a global pointer internally
...
- `SensirionI2cContextGuard` must wrap every Sensirion library operation that can call the HAL. It serializes access with a static FreeRTOS mutex, sets the active `i2c_dev_t*`, and clears it when the operation returns.
...
SPS30 communicates through the Sensirion vendor I2C HAL, adapted in `sensirion_i2c_hal.cpp`. The HAL translates `sensirion_i2c_hal_read/write` calls into `i2c_dev_read()` / `i2c_dev_write()` using the `i2c_dev_t*` selected by `SensirionI2cContextGuard`. The guard protects the otherwise global HAL context with a static FreeRTOS mutex so future Sensirion drivers cannot clobber the active device while a vendor call is in progress. Delays use `vTaskDelay` for ≥ 1 ms and `esp_rom_delay_us` for < 1 ms.
