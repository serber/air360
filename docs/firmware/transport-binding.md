...
| `applyDescriptorDefaults(dev, speed_hz)` | Applies the shared electrical policy — device clock speed and internal pull-up enables — to an `i2cdev` descriptor. The single author for per-device bus configuration: `setupDevice()` calls it internally, and every esp-idf-lib-based driver calls it on its descriptor right after `*_init_desc()`. |
