...
| `startup_calibration` | `uint8_t` | `0` | 0 or 1; only meaningful for sensor types whose descriptor sets `supports_startup_calibration` (currently SCD30, SCD40, and SCD41, where it toggles ASC). Reuses the former `reserved0` byte. |
| `pending_maintenance_action` | `uint8_t` | `0` | `MaintenanceActionKind` (0=none); a one-shot action run once after the next boot, then cleared. Validated against the sensor's advertised actions (SCD30/SCD40/SCD41: FRC; SPS30: fan clean). Carved from the former `reserved1` padding. See [sensors/maintenance-actions.md](sensors/maintenance-actions.md). |
...
| SCD40 / SCD41 | Same as SCD30: `startup_calibration` toggles ASC; `pending_maintenance_action` can schedule a one-shot FRC for the next boot |
