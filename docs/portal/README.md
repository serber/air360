...
with `chartjs-adapter-date-fns` supplying the time-scale tick math. The build
guide opens its photos in `yet-another-react-lightbox`, which supplies zoom,
keyboard navigation, and a slide counter.
